...
import { saveOffRouteDraft, getOffRouteDraft, clearOffRouteDraft } from "@/lib/offlineDb";
import { logOffRoute } from "@/lib/visitMachine";
...
export function OffRouteOrderCard({
  ...
  // Restore draft on mount (survives backgrounding)
  useEffect(() => {
    (async () => {
      try {
        const saved = await getOffRouteDraft();
        if (!saved) return;
  ...
  const submitOffRoute = async () => {
    ...
    try {
      // Single born-complete off_route event. No check-in, no photo, no
      // schedule link, no visit_events 'arrived' — never shows as in progress.
      await logOffRoute({
        repId,
        customerId: offRouteCustomerId,
      ...
      if (navigator.onLine) {
        ...
        if (r.synced > 0) onRefresh();
      } else {
        toast.success("Saved offline. Will sync when online.");
      }
      resetOffRoute();
    } catch (err: any) {
      console.warn("[OffRoute] submit error:", err?.message);
      toast.error("Failed to save. Please try again.");
    } finally {
      clearOffRouteDraft().catch(() => {});
    ...
    <div style={{ borderRadius: 22, overflow: "hidden", background: C.surface, border: `1.5px solid rgba(230, 182, 82, 0.35)` }}>
      ...
      <div style={{ padding: "16px", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 28, height: 28, borderRadius: 8, background: "rgba(230, 182, 82, 0.15)", display: "flex", alignItems: "center", justifyContent: "center", color: C.sun }}>
            <Plus size={14} />
          </div>
          <div>
            <p style={{ fontFamily: "Syne, sans-serif", fontSize: 14, fontWeight: 600, color: C.ink, margin: 0 }}>Off-Route order</p>
            ...
            offRouteSearchOpen ? (
              <div style={{ background: C.surface, borderRadius: 14, border: `1px solid ${C.border}`, overflow: "hidden" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", borderBottom: `1px solid ${C.border}` }}>
                  ...
                  <input type="text" value={offRouteSearch} onChange={(e) => setOffRouteSearch(e.target.value)} placeholder="Search customer..." style={{ flex: 1, border: "none", outline: "none", background: "transparent", fontFamily: "'DM Sans', sans-serif", fontSize: 14, color: C.ink }} />
                  <button type="button" onClick={() => { setOffRouteSearchOpen(false); setOffRouteSearch(""); }} style={{ background: "none", border: "none", color: C.inkMute, cursor: "pointer", padding: 0, flexShrink: 0 }}>
                    <X size={14} />
                  </button>
                </div>
                <div style={{ maxHeight: 200, overflowY: "auto" }}>
                  {[...adHocCustomers]
                    ...
                    .map(c => (
                      <button key={c.id} type="button" onClick={() => { setOffRouteCustomerId(c.id); setOffRouteSearchOpen(false); setOffRouteSearch(""); }} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "none", border: "none", borderBottom: `1px solid ${C.border}`, cursor: "pointer", textAlign: "left" }}>
              ...
              <button type="button" onClick={() => setOffRouteSearchOpen(true)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", borderRadius: 14, border: `1px solid ${C.border}`, background: C.surface, cursor: "pointer" }}>
                <Search size={14} style={{ color: C.inkMute, flexShrink: 0 }} />
        ...
        <div style={{ background: C.cream, borderRadius: 16, padding: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.3fr 0.6fr 1fr", gap: 8 }}>
            <label style={{ background: C.surface, borderRadius: 12, padding: "6px 10px", boxShadow: `inset 0 0 0 1px ${C.border}`, display: "block", cursor: "text" }}>
              ...
              <input value={offRouteOrderNumber} onChange={(e) => setOffRouteOrderNumber(e.target.value)} onBlur={resetMobileZoom} type="text" placeholder="Order #" inputMode="numeric" style={{ width: "100%", border: "none", outline: "none", background: "transparent", fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 14, color: C.ink, padding: 0, marginTop: 1 }} />
            </label>
            <label style={{ background: C.surface, borderRadius: 12, padding: "6px 10px", boxShadow: `inset 0 0 0 1px ${C.border}`, display: "block", cursor: "text" }}>
              ...
              <input type="number" min="0" step="1" value={offRouteOrderQty} onChange={(e) => setOffRouteOrderQty(e.target.value)} onBlur={resetMobileZoom} placeholder="0" inputMode="numeric" style={{ width: "100%", border: "none", outline: "none", background: "transparent", fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 14, color: C.ink, padding: 0, marginTop: 1 }} />
            </label>
            <label style={{ background: C.surface, borderRadius: 12, padding: "6px 10px", boxShadow: `inset 0 0 0 1px ${C.border}`, display: "block", cursor: "text" }}>
              ...
              <input type="text" min="0" step="0.01" value={offRouteOrderAmount} onChange={(e) => setOffRouteOrderAmount(e.target.value)} onBlur={resetMobileZoom} placeholder="0.00" inputMode="decimal" style={{ width: "100%", border: "none", outline: "none", background: "transparent", fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 14, color: C.ink, padding: 0, marginTop: 1 }} />
            </label>
          </div>
        </div>

        <div>
          <div style={{ fontSize: 10, color: C.inkMute, fontWeight: 700, letterSpacing: 1.2, textTransform: "uppercase", fontFamily: "'DM Sans', sans-serif", marginBottom: 6 }}>NOTES · OPTIONAL</div>
          <Textarea value={offRouteNotes} onChange={(e) => setOffRouteNotes(e.target.value)} onBlur={resetMobileZoom} rows={2} className="text-sm resize-none" style={{ borderColor: C.border, background: C.surface }} />
        </div>

        <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
          <Button type="button" onClick={resetOffRoute} className="flex-1 h-11 font-syne font-semibold" style={{ background: "transparent", color: C.inkSoft, border: `1px solid ${C.border}` }}>
            Cancel
          </Button>
          <ShimmerButton
            ...
            idleStyle={{ flex: 1, height: 44, borderRadius: 8, border: "none", background: `linear-gradient(135deg, ${C.greenMid} 0%, ${C.green} 100%)`, color: "#fff", fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}
