...
export const CameraCapture = forwardRef<HTMLButtonElement, CameraCaptureProps>(
  ...
  const closeCamera = () => {
    ...
    setReady(false);
    setError(null);
    // Absorb the ghost/compatibility click generated by the closing tap so it
    // cannot fall through to the check-out button beneath the overlay.
    setShielding(true);
    window.setTimeout(() => setShielding(false), 500);
  };
